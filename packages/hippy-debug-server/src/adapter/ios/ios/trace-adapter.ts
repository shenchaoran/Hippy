...
export default class TraceAdapter {
  ...
  getTraceEnd(trace: JscTrace, prevTrace: JscTrace) {
    ...
    const isSameFrame = (frame1, frame2) => frame1.sourceID === frame2.sourceID && frame1.line === frame2.line && frame1.column === frame2.column;
    ...
    for (let i = end, j = prevTrace.stackFrames.length - 1; i >= 0; i--, j--) {
      ...
      if (prevTraceFrame) if (isSameFrame(frame, prevTraceFrame)) {
        continue;
      } else {
        return i;
      }
